...
#include "semantic.h"  // Para a análise semântica
...
/* Programa: consiste de declarações globais e declarações de funções */
programa:
      lista_declaracoes_globais lista_declaracoes_funcoes
      { $$ = new_ast("programa", "", 2, $1, $2); ast_root = $$; }
    | lista_declaracoes_globais
      { $$ = new_ast("programa", "", 1, $1); ast_root = $$; }
    | lista_declaracoes_funcoes
      { $$ = new_ast("programa", "", 1, $1); ast_root = $$; }
    ;

/* Declarações globais: somente variáveis declaradas fora de funções */
lista_declaracoes_globais:
    ...
    | /* vazio */ 
      { $$ = new_ast("lista_declaracoes_globais", "vazio", 0); }
    ;
...
/* Nome da função: pode ser um ID ou o token MAIN */
nome_funcao:
      ID { $$ = new_ast("nome_funcao", $1, 0); }
    | MAIN { $$ = new_ast("nome_funcao", "main", 0); }
    ;
...
/* Comando composto: bloco delimitado por chaves */
comando_composto:
      ABRE_CHAVES declaracoes_locais lista_comandos FECHA_CHAVES 
         { $$ = new_ast("comando_composto", "", 2, $2, $3); }
    ;

/* Declarações locais (variáveis declaradas dentro de funções) */
declaracoes_locais:
...
/* Comando: pode ser uma expressão, bloco, seleção, iteração ou retorno */
comando:
...
lista_argumentos:
    ...
    | lista_argumentos VIRGULA expressao { $$ = new_ast("lista_argumentos", "", 2, $1, $3); }
...
expressao:
    ...
    | expressao_simples { $$ = $1; }
    | ID ABRE_PAREN lista_argumentos FECHA_PAREN { $$ = new_ast("chamada_funcao", "", 2, new_ast("ID", $1, 0), $3); }
    ;

/* Variável (uso) */
var:
      ID { $$ = new_ast("var", $1, 0); }
    | ID ABRE_COL expressao FECHA_COL { $$ = new_ast("array_access", "", 2, new_ast("ID", $1, 0), $3); }
    ;
...
int main() {
    ...
    if (yyparse() == 0) {
        ...
        // Aqui você pode chamar a análise semântica, se desejar:
